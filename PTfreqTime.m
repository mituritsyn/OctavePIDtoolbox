...
figure(PTspecfig3)
prop_max_screen=(max([get(PTspecfig3, 'Position')(3) get(PTspecfig3, 'Position')(4)]));
...
for i = 1 : 3
    ...
    if get(guiHandlesSpec3.ColormapSelect, 'Value')<8,
        colormap(char(get(guiHandlesSpec3.ColormapSelect, 'String')(get(guiHandlesSpec3.ColormapSelect, 'Value'))));
    end
    % new
    if get(guiHandlesSpec3.ColormapSelect, 'Value')==8, colormap(viridis); end
    if get(guiHandlesSpec3.ColormapSelect, 'Value')==9, colormap(linearREDcmap); end 
    if get(guiHandlesSpec3.ColormapSelect, 'Value')==10, colormap(linearGREYcmap); end
    cbar = colorbar('EastOutside');
    set(cbar.Label, 'String', 'Power Spectral density (dB)');
