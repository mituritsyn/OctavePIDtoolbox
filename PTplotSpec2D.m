...
if ~isempty(fnameMaster)
    %% update fonts
    prop_max_screen = (max([get(PTspecfig2, 'Position')(3) get(PTspecfig2,'Position')(4)]));
...
%todo lowess>moving 
                            ...
                            eval(['h=plot(freq2d2{p}.f' int2str(f) ', smooth(amp2d2{p}.f' int2str(f) ', log10(size(amp2d2{p}.f' int2str(f) ',1)) * (get(guiHandlesSpec2.smoothFactor_select, ''Value'')^3), ''moving''));hold on'])
                            hold on
    ...
    for m = 1:length(get(guiHandlesSpec2.SpecList, 'Value'))
        ...
        if ~isempty(freq2d2) &&~isempty(amp2d2)
            ...
            if get(guiHandlesSpec2.RPYcomboSpec, 'Value') == 0
                h = legend(legnd);
                set(h, 'Position', [0.88 0.33 - get(h, 'Position')(4) get(h, 'Position')(3:4)]);
            else
