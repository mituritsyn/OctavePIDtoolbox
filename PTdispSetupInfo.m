...
if ~isempty(fnameMaster) 
    if Nfiles < 2 
        str=strings(size(dataA(get(guiHandlesInfo.FileNumDispA, 'Value')).SetupInfo,1),1);
        str(:)=':'; str2=strcat(dataA(get(guiHandlesInfo.FileNumDispA, 'Value')).SetupInfo(:,1), char(str));
        setupA=strcat(str2, string(dataA(get(guiHandlesInfo.FileNumDispA, 'Value')).SetupInfo(:,2))); 
    else
        str=strings(size(dataA(get(guiHandlesInfo.FileNumDispA, 'Value')).SetupInfo,1),1);
        str(:)=':'; str2=strcat(dataA(get(guiHandlesInfo.FileNumDispA, 'Value')).SetupInfo(:,1), char(str));
        setupA=strcat(str2, string(dataA(get(guiHandlesInfo.FileNumDispA, 'Value')).SetupInfo(:,2))); 
        
        str=strings(size(dataA(get(guiHandlesInfo.FileNumDispB, 'Value')).SetupInfo,1),1);
        str(:)=':'; str2=strcat(dataA(get(guiHandlesInfo.FileNumDispB, 'Value')).SetupInfo(:,1), char(str));
        setupB=strcat(str2, string(dataA(get(guiHandlesInfo.FileNumDispB, 'Value')).SetupInfo(:,2)));
    end
